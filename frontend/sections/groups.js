;(function() {
  ...
  function editGroup(id) { if (typeof isAdmin === 'function' && !isAdmin()) { api.getCurrentUser().then(updatedUser => { appData.currentUser = { ...updatedUser, permissions: updatedUser.permissions || updatedUser.groupId?.permissions || [] }; const perms = appData.currentUser.permissions || []; if (!hasPermission('groups', perms, { allowAdminBypass: true })) { if (typeof showNotification === 'function') showNotification('You need admin privileges to edit groups', 'error'); return; } proceedWithEditGroup(id); }).catch(() => { if (typeof showNotification === 'function') showNotification('You need admin privileges to edit groups', 'error'); }); } else { proceedWithEditGroup(id); } }
